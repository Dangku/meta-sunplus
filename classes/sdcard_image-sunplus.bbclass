...
# Originally written for rasberrypi adapt for the needs of allwinner sunplus based boards
#
...
do_image_sunplus_sdimg[depends] += " \
...
# SD card image name
SDIMG = "${IMGDEPLOYDIR}/${IMAGE_NAME}.rootfs.sunplus-sdimg"

IMAGE_CMD_sunplus-sdimg () {
	...
	if [ -e "${DEPLOY_DIR_IMAGE}/${SPL_BINARY}" ]
	then
		mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${SPL_BINARY} ::${SPL_BINARY}
	fi

	if [ -e "${DEPLOY_DIR_IMAGE}/${UBOOT_BINARY}" ]
	then
		mcopy -i ${WORKDIR}/boot.img -s ${DEPLOY_DIR_IMAGE}/${UBOOT_BINARY} ::${UBOOT_BINARY}
	fi
